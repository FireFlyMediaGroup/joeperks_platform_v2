...
**I want** to implement the complete organization onboarding workflow with Stytch integration,
...
2. Stytch organization and admin user creation is automated during registration
...
- [ ] Integrate Stytch organization creation (AC: 2)
  - [ ] Implement automatic Stytch organization setup
  - [ ] Create admin user invitation workflow
  - [ ] Record Stytch organization ID mapping (persist stytch_org_id)
...
- [ ] Create feature flag management (AC: 6)
  ...
  - [ ] Add feature flag synchronization with Stytch
...
- [ ] Implement comprehensive testing (AC: All)
  ...
  - [ ] Mock external service testing (Stytch, Stripe, Maps)
  - [ ] Error handling and rollback scenario testing

## Dev Notes

### Previous Story Insights
**From Story 1.1**: Authentication service handles Stytch OAuth and organization context.  
...
sequenceDiagram
    ...
    K->>P: Return organization ID and admin invitation
...
- `admin_setup` - Stytch admin account created, awaiting activation
...
- `apps/medusa-server/src/services/onboarding.service.ts` - Main onboarding workflow orchestration
- `apps/medusa-server/src/services/geographic.service.ts` - Address validation and roaster matching
- `apps/medusa-server/src/services/partnership.service.ts` - Partnership approval workflow

**API Routes**:
- `apps/medusa-server/src/api/onboarding/` - Onboarding workflow endpoints
- `apps/medusa-server/src/api/onboarding/register/` - Registration endpoints
- `apps/medusa-server/src/api/onboarding/status/` - Status tracking endpoints

**Workflow State Management**:
- `apps/medusa-server/src/workflows/onboarding-state-machine.ts` - State transition logic
- `apps/medusa-server/src/workflows/approval-workflow.ts` - Partnership approval logic

### Technical Constraints
**External Service Integration** [Source: architecture/tech-stack.md#external-integrations]:
- **Stytch**: Multi-tenant auth, RBAC, and feature flags
...
- Feature flag synchronization between platform and Stytch
- Rollback capabilities for rejected organizations

### Integration Dependencies
**Foundation Services**:
- **Authentication Service (1.1)**: Stytch integration and JWT handling
...
- **External Service Mocking**: Mock Stytch, Stripe, and Google Maps APIs
- **Error Scenarios**: Test failure handling and rollback procedures

**Test File Locations**:
- Workflow tests: `apps/medusa-server/src/workflows/__tests__/onboarding.test.ts`
- Integration tests: `apps/medusa-server/src/api/onboarding/__tests__/`
- End-to-end tests: `apps/medusa-server/src/__tests__/onboarding-e2e.test.ts`
