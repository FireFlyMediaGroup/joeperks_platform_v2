...
- `GET /payments/{id}/status` - Check payment status and details
- `POST /stripe/webhooks` - Handle Stripe webhook events
- `GET /connect-accounts/{id}/status` - Check Connect account status

**Stripe Connect Endpoints**:
- `POST /connect/accounts/create` - Create Connect account for entity
- `GET /connect/accounts/{id}/onboarding-link` - Generate onboarding URL
- `PUT /connect/accounts/{id}/update` - Update account information
- `GET /connect/accounts/{id}/balance` - Check account balance and payouts

### File Locations
**Payment Services** [Source: architecture/source-tree.md#services]:
- `apps/api/src/services/payment.service.ts` - Core payment processing
- `apps/api/src/services/stripe-connect.service.ts` - Connect account management
- `apps/api/src/services/revenue-split.service.ts` - Revenue splitting logic
- `apps/api/src/services/webhook.service.ts` - Stripe webhook handling

**API Routes**:
- `apps/api/src/api/payments/` - Payment processing endpoints
- `apps/api/src/api/stripe/` - Stripe-specific endpoints and webhooks
- `apps/api/src/api/connect/` - Connect account management endpoints
...
**Test File Locations**:
- Service tests: `apps/api/src/services/__tests__/payment.service.test.ts`
- Webhook tests: `apps/api/src/api/stripe/__tests__/webhooks.test.ts`
- Integration tests: `apps/api/src/__tests__/payment-flow-e2e.test.ts`
- Security tests: `apps/api/src/__tests__/payment-security.test.ts`
