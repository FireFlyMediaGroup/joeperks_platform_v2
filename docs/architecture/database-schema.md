...
```sql
-- Core user management (integrates with Kinde)
CREATE TABLE users (
...
-- Organizations with Kinde integration
...
-- Organization members (synced with Kinde)
...
-- Coffee roasters with Kinde integration
CREATE TABLE roasters (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    kinde_org_code VARCHAR(255) UNIQUE NOT NULL, -- Kinde organization code for roaster
    ...
    feature_flags JSONB DEFAULT '{}', -- Cached from Kinde
    status VARCHAR(50) DEFAULT 'pending',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE customer_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    kinde_session_id VARCHAR(255) UNIQUE NOT NULL,
...
CREATE INDEX idx_users_kinde_id ON users(kinde_user_id);
CREATE INDEX idx_organizations_kinde_code ON organizations(kinde_org_code);
CREATE INDEX idx_org_members_org_user ON organization_members(organization_id, user_id);
CREATE INDEX idx_customer_sessions_kinde ON customer_sessions(kinde_session_id);
